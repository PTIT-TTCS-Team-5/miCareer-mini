...
def page_jobs():
    ...
    for j in jobs:
        with st.container():
            ...
            if st.button(f"Xem ứng viên ({j['jobpostid']})", key=f"job_{j['jobpostid']}"):
...
def page_applications():
    ...
    for a in apps:
        with st.container():
            ...
            with col1:
                st.write(f"**{a['fname']} {a['lname']}** ({a['email']}) - Trạng thái: `{a['stat']}` - Ngày nộp: {a['appliedat'].strftime('%Y-%m-%d') if a['appliedat'] else 'N/A'}")
            with col2:
...
def page_app_detail():
    ...
    with col2:
        ...
        model_tier = st.selectbox(
            ...
            ("Tier 1: Gemini Flash", "Tier 2: GPT-5.4 mini", "Tier 3: Claude 4.5 Haiku")
        )
        ...
        # Vùng nhập liệu mới
        if prompt := st.chat_input("Hãy phân tích ứng viên này so với Job Description?"):
